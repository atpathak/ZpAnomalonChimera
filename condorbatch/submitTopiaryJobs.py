...
if __name__=='__main__':
    ...
    #Print samples in json so ew know what we can work with
    if args.printsamples:
        print("printing the samples the are in the json, so you can pick one. Or just to look. Your call.")
    ...
    #Submit the jobs
    jobcnt = 0
    if len(fsjson.keys()) > 0 and not args.printsamples:
        for key in fsjson.keys():
            for samp in fsjson[key]:
                if args.sample:
